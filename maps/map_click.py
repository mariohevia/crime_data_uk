...
if "crime_data_clickable" not in st.session_state:
    st.session_state["crime_data_clickable"] = []
...
if st.session_state["selected_location_click"]:
    ...
    center = [lat,lon]
    zoom = 13
    st.session_state["crime_data_clickable"] = list_crimes_to_list_coordinates(get_crime_street_level_point(lat, lon))
else: 
    st.write("Selected location: ")

# Count and plot crime occurrences
crime_counts = Counter(st.session_state["crime_data_clickable"])
